...
class UnetConv3(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0), init_stride=(1,1,1)):
        ...
        # initialise the blocksg
        # self.children() generates children modules of nn modules.
        for m in self.children():
            init_weights(m, init_type='kaiming')
...
class UnetUp3_CT(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        # inputs2 is high-level feature-maps which has smaller size
        outputs2 = self.up(inputs2)
        # adapt the in-plane size of input1
        offset = outputs2.size()[2] - inputs1.size()[2]  # must be even
        padding = 2 * [offset // 2, offset // 2, 0]
        outputs1 = F.pad(inputs1, padding)
        return self.conv(torch.cat([outputs1, outputs2], 1))
