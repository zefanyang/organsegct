...
from models.exp_arch.unet_edge_deep_sup.unet_edge_ml_guidance_full_scheme import UNetEdgeMLGdnFullSch
...
parser.add_argument('--net', type=str, default='unet_edge_full_scheme')  # TODO
...
parser.add_argument('--beta1', type=float, default=1.)  # for DSC
parser.add_argument('--beta2', type=float, default=1.)  # for edge
parser.add_argument('--root', type=str, default='./output/unet_edge_full_scheme_wd')  # TODO
parser.add_argument('--cache_dir', type=str, default='./cache/cache_3')
parser.add_argument('--cv_json', type=str, default='/data/yzf/dataset/Project/ranet-dataset/cv_high_resolution.json')
...
def get_model(args):
    model = None
    if args.net == 'unet_edge_full_scheme':
        model = UNetEdgeMLGdnFullSch(1, args.num_class, init_ch=args.init_channels, interpolate=True)
...
def compute_loss(fnl_score, sec_score, trd_score, seg_score, seg, seg_one_hot):
    """Multi-level complementarity modeling loss, and information fusion loss"""
    # Multi-level complementarity modeling loss
    loss_fnl = [0., 0.]
    loss_sec = [0., 0.]
    loss_trd = [0., 0.]
    for score, loss in zip([fnl_score, sec_score, trd_score], [loss_fnl, loss_sec, loss_trd]):
        loss[0] = torch.nn.CrossEntropyLoss()(score, seg.squeeze(1).long())  # CE loss
        sm_output = F.softmax(score, dim=1)
        loss[1] = 1. - compute_per_channel_dice(sm_output, seg_one_hot).mean()  # Dice loss

    # sum
    loss_fnl = loss_fnl[0] + loss_fnl[1]
    loss_sec = loss_sec[0] + loss_sec[1]
    loss_trd = loss_trd[0] + loss_trd[1]

    # Information fusion loss
    loss_seg_ce = torch.nn.CrossEntropyLoss()(seg_score, seg.squeeze(1).long())
    seg_sm_output = F.softmax(seg_score, dim=1)
    seg_dice = compute_per_channel_dice(seg_sm_output, seg_one_hot)
    loss_seg_dice = 1. - seg_dice.mean() # Dice loss

    # sum
    loss_seg = loss_seg_ce + loss_seg_dice

    # Output seg maps
    predicted_map = torch.argmax(seg_sm_output, dim=1, keepdim=True).float()

    return loss_fnl, loss_sec, loss_trd, loss_seg, seg_dice, predicted_map
    ...
    # # final layer
    # loss_fnl_ce = torch.nn.CrossEntropyLoss()(fnl_score, seg.squeeze(1).long())
    # fnl_sm_output = F.softmax(fnl_score, dim=1)
    # loss_fnl_dice = 1. - compute_per_channel_dice(fnl_sm_output, seg_one_hot).mean()
    #
    # # second layer
    # loss_sec_ce = torch.nn.CrossEntropyLoss()(sec_score, seg.squeeze(1).long())
    # sec_sm_output = F.softmax(sec_score, dim=1)
    # loss_sec_dice = 1. - compute_per_channel_dice(sec_sm_output, seg_one_hot).mean()
    #
    # # third layer
    # loss_trd_ce = torch.nn.CrossEntropyLoss()(trd_score, seg.squeeze(1).long())
    # trd_sm_output = F.softmax(trd_score, dim=1)
    # loss_trd_dice = 1. - compute_per_channel_dice(trd_sm_output, seg_one_hot).mean()
...
def train_process_edge(epoch, args, net, optimizer, train_dataloader, writer=None):
    ...
    for i, tr_data in enumerate(train_dataloader):
        ...
        (fnl_score, sec_score, trd_score), seg_score, edge_score = net(volume)
        loss_fnl, loss_sec, loss_trd, loss_seg, seg_dice, predicted_map = compute_loss(*(fnl_score, sec_score, trd_score), seg_score, seg, seg_one_hot)
        loss_edge, edge_confidence = compute_edge_loss(edge, edge_score)

        # Overall loss function
        loss = (loss_fnl + loss_sec + loss_trd) + args.beta1 * loss_seg + args.beta2 * loss_edge

        loss_seg_meter.update(args.beta1 * loss_seg.item(), volume.shape[0])
        loss_edge_meter.update(args.beta2 * loss_edge.item(), volume.shape[0])
        dsc_meter.update((seg_dice[1:].mean()).item(), volume.shape[0])
...
def val_process_edge(epoch, args, net, val_dataloader, writer=None):
    ...
    with torch.no_grad():
        for i, val_data in enumerate(val_dataloader):
            ...
            val_loss_seg_meter.update(args.beta1 * loss_seg.item(), volume.shape[0])
...
def main():
    ...
    args.is_edge = True if 'edge' in args.net else False
