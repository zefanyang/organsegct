...
# The predicted edges look worse.
import os
import torch
...
class UNetEdgeB(nn.Module):
    ...
    def forward(self, x):
        ...
        _, edge_score = self.edge_module(enc3, mid)
...
class EGModule(nn.Module):
    def __init__(self, init_ch):
        ...
        self.h_conv = nn.Sequential(
            nn.Conv3d(init_ch * 8, init_ch * 4, 3, 1, 1),
            nn.BatchNorm3d(init_ch * 4),
            nn.ReLU()
        )
        self.e_conv = nn.Sequential(
            nn.Conv3d(init_ch * 4, init_ch * 4, 3, 1, 1),
            nn.BatchNorm3d(init_ch * 4),
            nn.ReLU(),
            nn.Conv3d(init_ch * 4, init_ch * 4, 3, 1, 1),
            nn.BatchNorm3d(init_ch * 4),
        ...
        self.out_conv = nn.Conv3d(init_ch*4, 1, 1)

    def forward(self, l_feat, h_feat):
        h_feat = self.h_conv(h_feat)
        h_feat = F.interpolate(h_feat, scale_factor=2, mode='trilinear')

        feat = h_feat + l_feat
        edge_feat = self.e_conv(feat)

        edge_score = self.out_conv(edge_feat)
        edge_score = F.interpolate(edge_score, scale_factor=4, mode='trilinear')
        return edge_feat, edge_score

if __name__ == '__main__':
 model = UNetEdge(1, 9, init_ch=16, conv_layer_order='cbr', interpolate=True)
