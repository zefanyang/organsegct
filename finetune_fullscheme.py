...
parser.add_argument('--pretrainedckp', required=True, help='path to the pre-trained model checkpoint')
...
def main_worker(args):
    ...
    # if args.resume:
    model = resume_model(model, args.pretrainedckp)
...
def main():
    ...
    args.root = f'./outputs/finetune/{args.net}_fold{args.fold}_{time.strftime("%H-%M-%S-%m%d")}'
